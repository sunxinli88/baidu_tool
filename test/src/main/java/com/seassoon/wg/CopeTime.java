...
//		bq = new LinkedBlockingQueue<>(getData);
		...
		bq = new LinkedBlockingQueue<>(pool);

		ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(50, 100, 100, TimeUnit.SECONDS,
...
class MyRunnable2 implements Runnable {
	...
	public void run() {
		...
		String sql, sql1 = "";
		while ((m = CopeTime.bq.poll()) != null) {

//			System.out.println("队列大小：【" + CopeTime.bq.size() + "】");
			...
			List<Map<String, Object>> result = CopeTime.cp.executeQuery(sql1, null);//excutQuery(sql1, null);
...
//			obj[4] = obj[3] + "|" + obj[0] + "md5";
			...
			if(params.size()%800==0){
				String tb = "wg_event_copy";
				CopeTime.cp.insert(CopeTime.cp,cols, params,tb);
				params.clear();
			}
